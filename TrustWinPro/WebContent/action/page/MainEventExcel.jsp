...
<div class="buttom" >
<a href="#EventInfo" onclick="MainEventExcel(document.getElementById('userlist1'),'<%=searchStartDate%>','<%=searchEndDate%>','<%=EventTime%>','<%=searchEndTime%>', '<%=searchName%>', '<%=searchUser %>', '<%=top %>');" class="button gray"><span class="icon-check"></span>Excel</a>
</div>
