...
<div id="userdata">
	...
	<div>
		<section class="sectionji">
			...
			<!--  class="searchForm" !-->
			<div>
				<form action="/TrustWinPro/action/index.jsp" name="allUser"
					id="allUser" method="post">
					<input type="hidden" value="User" name="left" /> <input
						type="hidden" value="AllUser" name="content" />
					<p>
					<div class="headerji">
						<%=Lanfunc.language(lan, 26)%>
						:
					</div>
					<input type="text" class="inputt" name="searchFirstName"
						value="<%=LangUtil.Empty(FirstName)%>" />
					</p>
					...
					<input type="text" class="inputt" name="searchMiddleName"
						value="<%=LangUtil.Empty(MiddleName)%>" />
					</p>
					...
					<input type="text" class="inputt" name="searchLastName"
						value="<%=LangUtil.Empty(LastName)%>" />
					</p>
					...
					<input type="text" class="inputt" name="searchCompanyID"
						value="<%=LangUtil.Empty(CompanyID)%>" />
					</p>
					...
					<input type="text" class="inputt" name="searchUserClass"
						value="<%=LangUtil.Empty(UserClass)%>" />
					</p>
					...
					<select name="searchDepartment" class="hourselect">
						...
						<option value="<%=cata[i].getIdx()%>"
							<%=cata[i].getIdx() == Department ? "selected" : ""%>><%=cata[i].getName()%></option>

						<%
							}
						%>
					</select> <input type="submit" name="submit"
						value="<%=Lanfunc.language(lan, 182)%>" class="ct-btn white"
  ...
  <span id="progressbar" style="width: 1%;" value="1">0%</span>
	...
	<div class="dropdown" style="margin-left: 1%; font-size: 13px">
		...
		<div id="myDropdown" class="dropdown-content">
			<a href="#" onclick="printPage();">Print</a> <a href="#"
			...
			<table cellspacing="0" class="titleEx1">
				...
				<tr>
					<th><%=Lanfunc.language(lan, 85)%> <input type="checkbox"
						name="allcheck" onclick="allUserInfoCheck();"
						style="z-index: 100;"></th>
				...
				<tbody>
					...
					%>	
						...
						<td class='date1 <%=statuss2%>'><a href="#a"
							onclick="submitUser('User','<%=users[i].getUserId()%>')"><%=users[i].getMiddleName()%></a></td>
						<td class='date1 <%=statuss3%>'><a href="#a"
							onclick="submitUser('User','<%=users[i].getUserId()%>')"><%=users[i].getLastName()%></a></td>
						<td class='date1 <%=statuss4%>'><a href="#a"
							onclick="submitUser('User','<%=users[i].getUserId()%>')"><%=users[i].getUserId()%></a></td>
		...
		<div class="bottom bt_user">
			...
			<a href="#" title="Send" onclick="checkedF(<%=users.length%>,1);"
				class="button yellow"><span style="margin-left: 0px;"><img
					src="/TrustWinPro/action/image/interface/communication.png"
					style="height: 20px; width: 20px; border-radius:3px;"></span>
			<%-- <%=Lanfunc.language(lan, 183)%> --%>Send</a> <a href="#"
				title="Recive" onclick="checkedF(<%=users.length%>,3);"
				class="button yellow"><span style="margin-left: 0px;"><img
					src="/TrustWinPro/action/image/interface/communication.png"
					style="height: 20px; width: 20px; border-radius:3px;"></span><%=Lanfunc.language(lan, 184)%></a>
			<a href="#" title="Delete" onclick="checkedF(<%=users.length%>,2);"
				class="button yellow"><span style="margin-left: 0px;"><img
					src="/TrustWinPro/action/image/interface/communication.png"
					style="height: 20px; width: 20px; border-radius:3px;"></span><%=Lanfunc.language(lan, 88)%></a>
		</div>
	</form>
	...
	<div class="postitUserSort" id="postitUserSort" style="display: none">
		<jsp:include page="UserSort.jsp" flush="true">
			<jsp:param name="searchFirstName"
				value="<%=LangUtil.Empty(FirstName)%>" />
			<jsp:param name="searchMiddelName"
				value="<%=LangUtil.Empty(MiddleName)%>" />
			<jsp:param name="searchLastName"
				value="<%=LangUtil.Empty(LastName)%>" />
			<jsp:param name="searchUserclass"
				value="<%=LangUtil.Empty(UserClass)%>" />
			<jsp:param name="searchCompanyID"
				value="<%=LangUtil.Empty(CompanyID)%>" />
			<jsp:param name="searchDepartment" value="<%=Department%>" />
			<jsp:param name="searchName" value="accessGroup.getName()" />
			<jsp:param name="searchContollerName"
