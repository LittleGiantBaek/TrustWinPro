...
<%
	if(request.getParameterValues("check") != null){
		...
		for(int i=0;i<date.length;i++){
			String used = "";
			String sql = "select Used from Holidays where HolidayDate = '" + date[i] + "'"; 
			try {
			...
			if(used.equals("1") ){
				sql = "Update Holidays set Used = '0' where HolidayDate = '" + date[i] + "'";
			}else{
				sql = "Update Holidays set Used = '1' where HolidayDate = '" + date[i] + "'";
			}
			try {
					Context init = new InitialContext();
					DataSource ds = (DataSource)init.lookup("java:comp/env/jdbc/MssqlDB");
